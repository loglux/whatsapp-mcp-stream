  ...
  server.tool(
    ...
    "Get WhatsApp chats, optionally filtered and sorted.",
    {
      ...
      include_last_message: z
        ...
        .optional()
        .default(true)
        .describe("Whether to include the last message details"),
  ...
  server.tool(
    ...
    "Get WhatsApp chat metadata by JID.",
    {
      jid: z
        .string()
        .describe(
          "The JID of the chat to retrieve (e.g., 123456789@s.whatsapp.net or 123456789-12345678@g.us)",
        ),
      ...
      try {
        ...
        return {
          content: [
            {
              type: "text",
              text: `Error getting chat ${jid}: ${error.message}`,
            },
          ],
          isError: true,
        };
  ...
  server.tool(
    ...
    "Resolve a contact by name or phone number and return the chat metadata.",
    {
      ...
      max_candidates: z
        ...
        .optional()
        .default(5)
        .describe("Maximum number of matches to consider"),
    },
    async ({ query, max_candidates }): Promise<CallToolResult> => {
      try {
        const candidates = await whatsappService.resolveContacts(
          query,
          max_candidates,
        );
        if (!candidates.length) {
          return {
            content: [
              { type: "text", text: `No contacts matched for query: ${query}` },
            ],
            isError: true,
          };
        }
        ...
        if (!chat) {
          return {
            content: [
              {
                type: "text",
                text: JSON.stringify({ contact: top, chat: null }, null, 2),
              },
            ],
          };
        }
        ...
        log.error(
          `Error in get_chat_by_contact tool for query ${query}:`,
          error,
        );
        return {
          content: [
            {
              type: "text",
              text: `Error getting chat for ${query}: ${error.message}`,
            },
          ],
          isError: true,
        };

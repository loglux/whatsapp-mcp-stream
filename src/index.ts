...
const cliTransport = process.argv.includes('--stdio')
  ? 'stdio'
  : process.argv.includes('--sse')
    ? 'sse'
    : (process.argv.includes('--http') || process.argv.includes('--streamable-http'))
      ? 'http'
      : null;

const envTransport = (process.env.TRANSPORT || '').toLowerCase();
const resolvedEnvTransport =
  envTransport === 'sse'
    ? 'sse'
    : (envTransport === 'http' || envTransport === 'streamable_http' || envTransport === 'streamable-http')
      ? 'http'
      : envTransport === 'stdio'
        ? 'stdio'
        : null;

const resolvedTransport = cliTransport || resolvedEnvTransport || 'stdio';
...
async function main() {
  ...
  const transportType = resolvedTransport;

 ...
 */
async function findChromeBrowsers(): Promise<
 ...
 */
function identifyWhatsAppChromeBrowsers(
...
async function cleanupOrphanedBrowsers(): Promise<void> {
  ...
  const whatsAppChromeProcesses =
    identifyWhatsAppChromeBrowsers(allChromeProcesses);
  log.info(
    `Identified ${whatsAppChromeProcesses.length} Chrome processes that might be related to WhatsApp`,
  );
  ...
  whatsAppChromeProcesses.forEach((process) => {
    log.info(
      `PID ${process.pid}: ${process.command.substring(0, 100)}${process.command.length > 100 ? "..." : ""}`,
    );
  });
  ...
  log.info(
    `Cleanup complete. ${killedCount} of ${whatsAppChromeProcesses.length} processes killed.`,
  );
}

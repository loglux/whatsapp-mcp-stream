...
export class BrowserProcessManager {
  ...
  constructor() {
    ...
    log.info(
      `Initialized BrowserProcessManager with instance ID: ${this.serverInstanceId}`,
    );
  }
  ...
  async cleanupOrphanedProcesses(): Promise<void> {
    ...
    for (const process of processes) {
      ...
      if (isRunning) {
        ...
        const isFromCurrentInstance =
          process.serverInstanceId === this.serverInstanceId;
    ...
    log.info(
      `Cleanup complete. ${processes.length - validProcesses.length} orphaned processes removed.`,
    );
  }
}
